...
function instantiate_4th_order_2nd_pade_model(system::PSY.System; solve_powerflow = false)
    nl_sys = get_4th_order_2nd_pade_nonlinear_system()
    variable_count = length(states(nl_sys))
    nlsys_func = MTK.generate_function(nl_sys, expression = Val{false})[2]
    sys_f = (out, x, param) -> nlsys_func(out, x, param)
...
function instantiate_reduced_dc_model(system::PSY.System; solve_powerflow = false)
    nl_sys = get_reduced_dc_nonlinear_system()
    variable_count = length(states(nl_sys))
    nlsys_func = MTK.generate_function(nl_sys, expression = Val{false})[2]
    sys_f = (out, x, param) -> nlsys_func(out, x, param)
