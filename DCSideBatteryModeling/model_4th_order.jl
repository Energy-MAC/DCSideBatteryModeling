...
function dae_model_4th_order(::Nothing)
    ...
    model_rhs = [
        ...
        (ωb/cdc) * (i_in - p_inv / (vdc))
        #∂ibat/∂t
        (ωb / ldc) * (vb - vc1 - vc2 - rl2*(ibat-il2) - rl1*(ibat-il1) - rb0 * ibat - (1 - d_dc) * vdc)
        #∂il1/∂t
        (ωb / lb1) * (rl1*(ibat-il1))
        #∂il2/∂t
        (ωb / lb2) * (rl2*(ibat-il2))
        ...
        i_ref + p_inv / (vdc) - i_in # Integrator for DC/DC inner PI controller
        # ∂A/dt
        (a1/Ts)*A + (a2/Ts^2)*B + kpib * (i_ref + p_inv / (vdc) - i_in) + kiib * κ + kpred*i_pred
        # ∂B/dt
        A # Second term in 2nd-order Pade approx.
        # ∂C/dt
        (b1/Ts)*C + (b2/Ts^2)*D + (b3/Ts^3)*E + kpib * (i_ref + p_inv / (vdc) - i_in) + kiib * κ + kpred*i_pred
        # ∂D/dt
        C 
        # ∂E/dt 
        D 
